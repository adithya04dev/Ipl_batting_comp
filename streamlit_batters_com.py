...
def main():
    # Title of the app
    st.title("IPL: Batters Comparision ")
    ...
    if st.button('Submit'):
        ...
        st.write(" Note : Players with infinite average(0 outs) aren't plotted on graph")
        ...
        result=batcomp.calculate(overs,bowling_type,Season,min_balls)
        ...
        for i, row in result1.iterrows():
            scatter_plot.text(row['strike_rate'],row['average_runs'], row['player_name'], fontsize=18, alpha=0.7)
